...
// =============================================
// DYNAMIC SERVICES CAROUSEL – POWERED BY SITEPINS
// =============================================
fetch('../data/site-content.json')
  .then(response => response.json())
  .then(data => {
    const services = data.servicesCarousel.items;
    ...
    services.forEach((service, index) => {
      ...
      const cardClass = isRight ? 'sc-card sc-card--br-right' : 'sc-card sc-card--br-left';

      let includesHTML = '';
      service.includes.forEach(item => {
        includesHTML += `<div class="sc-inc-item">${item}</div>`;
      });
      ...
      const cardHTML = `
        <div class="${cardClass}">
          ...
          <div class="sc-card__body">
            <h3 class="sc-card__title">${service.title}</h3>
            <p class="sc-card__desc">${service.desc}</p>
            <span class="sc-inc-label">Includes</span>
            ${includesHTML}
          </div>
        </div>
      `;
    ...
    // Re-run the original carousel logic after cards are added
    initCarousel();
  })
  .catch(err => console.error('Services carousel load error:', err));

// =============================================
// Original carousel logic (slightly adjusted to be callable)
// =============================================
